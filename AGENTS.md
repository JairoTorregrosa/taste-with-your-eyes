...
31. The Agent must use Tailwind's utility-first approach and keep design tokens in Tailwind config or Tailwind v4 `@theme` definitions. ￼
...
36. The Agent must use Bun as:
    - Package manager (`bun install`).
    - Script runner (`bun run`).
    - Test runner (`bun test`), where applicable. ￼

37. Project scripts must, by default, be defined in terms of Bun, while remaining usable on Node-based CI when reasonable.

38. The Agent must adopt Biome as the single source of truth for:
    - Formatting.
    - Linting.
    - Basic static analysis. ￼

39. ESLint and Prettier should not be introduced unless user constraints require them; in that case, the Agent must avoid overlapping responsibilities with Biome.

40. The Agent must enforce:
    - Lint and format on CI.
    - Local commands such as `bun run lint`, `bun run format`, `bun run check`.
    - The `check` script must run: `biome check && tsc --noEmit` to verify both linting/formatting and type checking.

---

### 10. Security and Reliability

41. The Agent must keep abreast of security advisories for:
    - React and React Server Components. ￼
    - Next.js CVEs and patch releases. ￼

42. When a known critical vulnerability exists in the stack, the Agent must:
...
44. The Agent must design for graceful failure:
...
45. The Agent must write tests for:
...
46. Where feasible, the Agent should use Bun's built-in test runner for unit and integration tests. ￼

47. The Agent must ensure that:
...
48. Before producing code, the Agent must:
...
49. After producing code, the Agent must perform a self-review:
...
50. When the Agent detects violations of this constitution in user-provided code, it must:
    - Call out the violations explicitly.
    - Suggest concrete refactors that move the code closer to constitutional compliance.

51. This constitution may be updated when:
...
52. Until amended, the Agent must treat this document as binding on all autonomous coding behavior within the defined stack.